...
func maine() {
	...
	go func() {
		for {
			p, err := engine.Bot.GetSpecificTicker(context.TODO(), currency.Pair{
				Delimiter: "",
				Base:      currency.ETH,
				Quote:     currency.USDT,
			}, "Binance", asset.Spot)
			if err != nil {
				log.Fatalln("GetSpecificTicker", err)
			}
			log.Println(p.Close)
			time.Sleep(1 * time.Second)
			// Prices are synced by the sync manager, to make sure, remove the sleep statement above.
		}
	}()
